...
class TestGenerate(unittest.TestCase):
    ...
    def test_bigger_box_trends_more_expensive(self):
        rows = generate(num_rows=1000, seed=1)
        small = [r for r in rows if r["width_in"] <= 1.0 and r["length_in"] <= 3.0]
        big = [r for r in rows if r["width_in"] >= 15 and r["length_in"] >= 20]
...
class TestQuoteLookup(unittest.TestCase):
    ...
    def test_finds_matches_for_sampled_size(self):
        # ~Z96-128A (6x8) -- a real sampled catalog size.
        matches = find_similar_quotes(self.quotes, width_in=6, length_in=8, height_in=4)
    ...
    def test_estimate_has_positive_price_and_confidence(self):
        estimate = estimate_from_history(self.quotes, width_in=6, length_in=8, height_in=4)
