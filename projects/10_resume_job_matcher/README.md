...
| `job_search.py` | `search_jobs(keywords, location, radius_miles, count)` → live postings via Claude's `web_search` (restricted to `job_sites`) + `web_fetch` (full posting text). Returns `{title, company, location, url, description, grounding}`; `grounding` is `"full posting"` or `"search snippet"`. `fetch_posting(url)` — the manual escape hatch: `web_fetch` one URL, return its job text (or `""` if it's a results page / dead listing). |
...
- **The URL isn't always canonical.** For an aggregator result the model
  ...
  permalink. The Streamlit result dialog has a **Re-fetch & re-score** box:
  paste the posting's own URL and `job_search.fetch_posting` reads that page
  and rescores against its full text (grounding becomes `"manual fetch"`).
...
python -m unittest discover   # 148 tests, offline
